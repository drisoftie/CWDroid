...
public class CwApi {
    ...
    private static CwApiParser<XmlNews>  newsParser;
    ...
    public static List<CwNews> getNewsList(int count) {
        return getNewsList(count, null);
    }

    public static List<CwNews> getNewsList(int count, Integer filter) {
        List<CwNews> result = null;
        if (newsParser == null) {
            newsParser = new CwApiParser<>(XmlNews.class);
        }
        try {
            CwApiParser<XmlNews>.Builder builder = newsParser.build().setCommand(CwApiCommand.GET_NEWS_LIST).addArgument(
            ...
            XmlNews root = (XmlNews) builder.finish().parse();
            if (root != null && !root.getNews().isEmpty()) {
                result = root.getNews();
            }
        } catch (IOException e) {
            e.printStackTrace();
